...
# Set up run outputs
#-----------------------------------------------------------------------------
exec mkdir "../outputs_1soil_3D_random"
cd "../outputs_1soil_3D_random"

# path to clm_input
set clmdir      "../../Parflow_Preprocessing/Distributed_station166_met"
...
for {set i 0} {$i <= $stoptime} {incr i} {
	foreach v $var {
 	set filename [format "%s/%s.%s.%06d_to_%06d.pfb" $clmdir $metfile $v $i $i]
 # puts $filename
	pfdist $filename
 #pfundist $filename
}
